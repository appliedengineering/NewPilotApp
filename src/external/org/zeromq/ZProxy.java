...
import external.org.zeromq.ZActor.Actor;
import external.org.zeromq.ZAgent.SelectorCreator;
import external.org.zeromq.ZMQ.Socket;
import external.org.zeromq.ZStar.Exit;
    ...
    private static final class ZmqPump implements Pump
    {
        ...
        public boolean flow(Plug splug, Socket source, Socket capture, Plug dplug, Socket destination)
        {
            ...
            while (true) {
                ...
                long more = src.getSocketOpt(external.zmq.ZMQ.ZMQ_RCVMORE);
                ...
                if (cpt != null) {
                    ...
                    rc = cpt.send(ctrl, more > 0 ? external.zmq.ZMQ.ZMQ_SNDMORE : 0);
                    if (!rc) {
                        // not successful, but we can still try to send it to the destination
                    }
                }
                ...
                // we send the packet
                rc = dst.send(msg, more > 0 ? external.zmq.ZMQ.ZMQ_SNDMORE : 0);
